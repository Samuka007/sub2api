...
分支名包含 `/`，必须把完整 ref 传给 Git。GitHub 的 `/tree/otel/model-trace` 页面可能被解释为分支 `otel` 下的 `model-trace` 路径，不能作为 branch 解析依据。以下命令中的 `BRANCH` 必须替换为实际分支名：

```bash
BRANCH='otel/model-trace'
git clone --branch "$BRANCH" --single-branch https://github.com/Alle-Group/sub2api.git
git -C sub2api rev-parse HEAD
```
...
sub2api 启动时只设置 tracing endpoint、公钥、秘密和 `capture_media_content=false`，**不设置**三个 `MODEL_TRACING_*_MAX_BYTES` 环境变量。管理员 GET 首次必须返回部署来源：`prompt_max_bytes=16777216`、`response_max_bytes=8388608`、`media_max_bytes=16777216`。
...
2. 切到三个 `1048576`，生成 1,040,000 字节正文（完整 JSON 请求体仍不超过 1 MiB），经真实 OTLP 写入 Langfuse；ClickHouse 必须同时保留 head/tail canary、存储长度在 `[1040000, 1048576]`，且不存在截断标记。

这两步验证的是“部署默认可观察 + 运行时小上限真实生效 + 运行时 1 MiB 上限可真实接收近边界请求”，不能用静态配置或 fake exporter 代替。
