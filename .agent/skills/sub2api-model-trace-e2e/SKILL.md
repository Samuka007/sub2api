...
3. **远端用 GitHub**。本仓库 `origin` = `git@github.com:Alle-Group/sub2api.git`，`upstream` = `git@github.com:Wei-Shaw/sub2api.git` 且只读；Pull Request 只提交到 `origin`。**禁止**触发 `antcode-skill`，本仓库与 AntCode 无关。
...
2. 确认宿主端口 3000、15432、16379、18081、8080、5432、6379 未被占用。脚本先检查端口，再清理当前 checkout 以 owner label 标记的旧资源；不会停止其他 checkout 或普通 Docker 项目。
...
1. 从仓库根目录串行运行：
   ```bash
   bash .agent/skills/sub2api-model-trace-e2e/scripts/run_full_e2e.sh
   ```
...
所有命令从仓库根目录运行，`SKILL_DIR=.agent/skills/sub2api-model-trace-e2e`。
