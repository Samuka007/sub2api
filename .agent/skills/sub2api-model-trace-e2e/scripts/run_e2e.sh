...
for container_name in \
  ...
  sub2api-langfuse-clickhouse-1 sub2api-langfuse-minio-1; do
...
echo "$DEPLOYMENT_CONFIG" | jq -e '
  ...
  .data.prompt_max_bytes == 16777216 and
  .data.response_max_bytes == 8388608 and
  .data.media_max_bytes == 16777216 and
...
# 6.1 小上限截断完成后显式切到 1 MiB，并发送接近但不超过上限的真实 Prompt。
log "setting runtime limits to 1 MiB for the near-limit request"
ONE_MIB_LIMIT_CONFIG=$(jq -nc \
  --arg endpoint "$LANGFUSE_TRACE_ENDPOINT" \
  --arg public_key "$LANGFUSE_PK" \
...
ONE_MIB_LIMIT_RESPONSE=$(curl -fsS -X PUT \
  -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
  -d "$ONE_MIB_LIMIT_CONFIG" "$CONFIG_URL")
echo "$ONE_MIB_LIMIT_RESPONSE" | jq -e '
  .data.source == "runtime" and
  .data.has_secret == true and
...
[[ "$ONE_MIB_LIMIT_RESPONSE" != *"$LANGFUSE_SK"* ]] || fail "1 MiB runtime update response leaked secret material"
