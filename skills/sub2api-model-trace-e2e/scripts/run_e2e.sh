...
# 6.3 在原始账号组挂成功账号，验证真实 Anthropic SSE 到客户端终态。
STREAM_ACCOUNT_PAYLOAD=$(jq -nc --argjson group_id "$GROUP_ID" '{
  name:"e2e-stream-account", platform:"anthropic", type:"apikey", concurrency:1, priority:1,
  group_ids:[$group_id], credentials:{api_key:"e2e-upstream-stream-key", base_url:"http://127.0.0.1:18081/ok", model_mapping:{"gpt-4":"claude-e2e"}}, extra:{}
}')
STREAM_ACCOUNT_ID=$(curl -fsS -X POST -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
  -d "$STREAM_ACCOUNT_PAYLOAD" http://localhost:8080/api/v1/admin/accounts | jq -r '.data.id')
[[ -n "$STREAM_ACCOUNT_ID" && "$STREAM_ACCOUNT_ID" != "null" ]] || fail "stream Anthropic account creation failed"
...
BATCH_SUBMIT_PAYLOAD=$(jq -nc --arg task "$BATCH_TASK_NAME" '{
  model:"gemini-2.5-flash-image", task_name:$task, provider:"gemini_api", image_size:"1K",
  items:[
    {custom_id:"item-a",prompt:"e2e batch successful item"},
    {custom_id:"item-b",prompt:"e2e batch failed item"}
  ]
}')
...
echo "$BATCH_GET_RESPONSE" | jq -e '.item_count == 2 and .success_count == 1 and .fail_count == 1 and .actual_cost == 0' >/dev/null \
  || fail "batch terminal counters or zero-cost settlement mismatch"
BATCH_ITEMS_RESPONSE=$(curl -fsS \
  -H "Authorization: Bearer $BATCH_APIKEY" \
  "http://localhost:8080/v1/images/batches/$BATCH_ID/items")
echo "$BATCH_ITEMS_RESPONSE" | jq -e '
  (.data | length) == 2 and
  (any(.data[]; .custom_id == "item-a" and .status == "succeeded" and .image_count == 1)) and
  (any(.data[]; .custom_id == "item-b" and .status == "failed" and .error.code == "SAFETY_BLOCKED"))
' >/dev/null || fail "batch item terminal semantics mismatch"
BATCH_FIXTURE_STATS=$(curl -fsS http://localhost:18081/stats)
echo "$BATCH_FIXTURE_STATS" | jq -e '
  .batch_auth_fail == 0 and .batch_auth_ok >= 5 and .batch_input_items == 2 and
...
log "batch API and worker verified: http=$BATCH_CODE status=$BATCH_STATUS items=2 success=1 failed=1 upstream_auth=verified"
...
# 8.6 真实异步 batch：提交根 Span 与 worker 的两个 Generation 共享一个 Trace，续接及终态正确。
...
[[ "$BATCH_TRACE_COUNT" == "1" && "$BATCH_OBS_COUNT" == "3" && "$BATCH_ROOT_COUNT" == "1" && "$BATCH_GEN_COUNT" == "2" && "$BATCH_DIRECT_CHILDREN" == "2" && "$BATCH_CONTINUED_COUNT" == "2" && "$BATCH_COMPLETED_COUNT" == "1" && "$BATCH_FAILED_COUNT" == "1" && "$BATCH_UNFINISHED_COUNT" == "0" ]] \
  || fail "batch trace mismatch: traces=$BATCH_TRACE_COUNT observations=$BATCH_OBS_COUNT root=$BATCH_ROOT_COUNT generations=$BATCH_GEN_COUNT direct=$BATCH_DIRECT_CHILDREN continued=$BATCH_CONTINUED_COUNT completed=$BATCH_COMPLETED_COUNT failed=$BATCH_FAILED_COUNT unfinished=$BATCH_UNFINISHED_COUNT"
...
[[ "$SENSITIVE_HITS" == "0" ]] || fail "Langfuse stored $SENSITIVE_HITS observations containing a credential or media canary"

# 9. 输出
echo "$TRACE_ID"
echo "$OBS_COUNT"
echo "VERIFY_OK"
log "http statuses: langfuse=200 sub2api=200 anonymous=$ANON_CODE unknown=$UNKNOWN_CODE control=$CONTROL_CODE recognized=$HTTP_CODE disabled=$DISABLED_CODE secure=$SECURE_CODE near_limit=$LARGE_CODE failover=$FAILOVER_CODE stream=$STREAM_CODE batch=$BATCH_CODE"
...
log "batch trace passed: trace=$BATCH_TRACE_ID observations=$BATCH_OBS_COUNT generations=$BATCH_GEN_COUNT direct_async_children=$BATCH_DIRECT_CHILDREN completed=1 failed=1 status=$BATCH_STATUS"
log "sensitive-content gate passed: credential_or_media_hits=$SENSITIVE_HITS"
log "e2e smoke passed: trace=$TRACE_ID observations=$OBS_COUNT; anonymous/unknown/control=0 disabled=1"