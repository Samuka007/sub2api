...
`v0.1.183` 在 `v0.1.182` 基线上集中修复 OpenAI、Kimi、Antigravity、邮箱绑定及频道监控等
场景的调度、会话与兼容性问题：OpenAI OAuth 识别 5 小时/7 天配额耗尽的 429 并按重置时间暂停
账号（普通瞬时 429 继续重试）；Kimi 并发限制 403 改为临时冷却并保留故障转移；OpenAI 支持
Codex `session-id` 请求头保持重连粘性会话、容量溢出时不迁移持久绑定、修正 custom tool/tool
search 调用恢复后的项目 ID 前缀；邮箱换绑支持别名占用检测与事务级并发保护；Antigravity 兼容
模式最大 token 限制为 64000；频道监控 v2 修复 composite 平台错误聚合的 SQL 条件。
