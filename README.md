...
```bash
git diff v0.1.178..HEAD
```

## 上游 v0.1.178 文档与功能

本仓库以这份 README 作为 4Sub2 私有功能和团队协作的主说明。同步 `v0.1.178` 时，
...
`v0.1.178` 在 `v0.1.177` 基线上新增 Kimi/智谱/DeepSeek 供应商支持（多协议接入，含分组创建入口、
渠道定价与配额/余额监控）、渠道监控配额快照模式（覆盖 8 个平台，公开开关默认关闭）、渠道模型
谷峰定价（token 计费渠道按时段配置价格倍率，`224`/`225`/`226` 迁移）、OpenAI Team 账号联动熔断、
OpenAI 账号批量设置、Grok 用量条补齐本站 24h/7d/30d 聚合、Ollama 账号用量查询、Ops 错误详情
自定义时间区间；统一 Codex OAuth 出站身份（凭据面与真实客户端对齐，消除模型版本漂移）、透传
WS 二进制帧纳入策略链（模型映射、安全审计）、透传会话谷峰倍率取值时刻改为 turn 开始时刻、仪表盘
Token 卡片明细纳入缓存 token；并修复邀请码并发注册竞争（消费与用户创建原子化）、Kimi/智谱/
DeepSeek 分组的调度闸门/计费/断开漏记/count_tokens/403、Gemini 参数丢失与 4xx 硬改 500、
Anthropic SSE 过载、认证快照丢失分组定价、Claude deferred tools cache_control 等缺陷。
