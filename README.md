...
```bash
git diff v0.1.179..HEAD
```

## 上游 v0.1.179 文档与功能

本仓库以这份 README 作为 4Sub2 私有功能和团队协作的主说明。同步 `v0.1.179` 时，
...
`v0.1.179` 在 `v0.1.178` 基线上新增账号自适应 API 协议（按账号能力自动选择上游协议）、
渠道定价服务层级与上下文区间倍率、原生 Anthropic Fast mode 计费、Composite 支持 Codex
与国产供应商、代理探测目标可配置、`/v1/responses/input_tokens` 预检；并将用量统计切换为
`GROUPING SETS` 聚合；同时修复了上游同步引入的渠道调度、计费与错误透传相关缺陷。
