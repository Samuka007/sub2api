...
- [ ] 7.3 [Requirement: 会话事件记录不得依赖 Langfuse 历史；Scenario: 重复历史仍追加为会话事件；Scenario: 模型请求不读取 Langfuse Public API；Scenario: 客户端显式声明上下文压缩；Scenario: 离线导出重建去重 transcript] 先为 HTTP 与 Responses WebSocket 写 RED 合约测试：fake OTLP server 必须拒绝所有 Langfuse Public API GET，重复 `message_id` 的后续回合仍产生 `chat.*` Span，显式 `request_kind=compaction` 仍产生 `chat.compact`；再删除生产 read-back、相关 reader 和仅覆盖该行为的测试，令会话事件只基于当前 input/output 追加，并保留离线导出按最早 `message_id` 去重。进入下一模型追踪 task 前更新 `docs/MODEL_TRACING_IMPLEMENTATION_LOG.md`，记录去除的运行时依赖和验证证据；验证：`cd backend && go test ./internal/modeltrace/... -run 'TestModelTraceConversation|TestModelTraceResponsesWSTurn' -count=1 && cd .. && python3 scripts/test_langfuse_session_export.py`，预期目标测试通过、fake server 未观察到 Public API GET，导出测试保留最早事件。
## 8. 扩展全部同步协议与入口
