...
- 对具有显式 Session 标识的会话请求，将本轮可解析的用户、系统、助手和工具事件以稳定 `message_id` 追加为 OTLP `chat.*` 观察；模型请求路径不得查询 Langfuse Public API 历史来跳过事件、判断压缩或抑制 fork 标记。重复事件由离线导出在数据抵达 Langfuse 后按 `message_id` 去重。
...
- `langfuse-otel-export`：定义自部署 Langfuse OTLP 目标、部署与运行时配置优先级、秘密保护、旁路失败行为、会话事件仅追加 OTLP 的边界，以及管理配置契约。
