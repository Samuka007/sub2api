...
func (h *SettingHandler) UpdateSettings(c *gin.Context) {
	...
	} else if req.AliyunCaptchaRegion != service.AliyunCaptchaRegionCN && req.AliyunCaptchaRegion != service.AliyunCaptchaRegionSGP {
		response.BadRequest(c, "Aliyun Captcha region must be cn or sgp")
		return
	}
	...
	// Turnstile 参数验证。部分更新先合并已存字段，再按有效开关状态校验。
	...
	if aliyunCaptchaEnabled {
		...
		if credentialsChanged {
			if h.aliyunCaptchaService == nil {
				response.BadRequest(c, "Aliyun Captcha service is not configured")
				return
			}
