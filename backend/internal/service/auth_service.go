...
// VerifyCaptchaForPendingOAuthCreate 避免在邮箱验证码流程中重复消费 Turnstile token。
// 腾讯天御与阿里云验证码是动作级 proof，创建 pending OAuth 账号时仍须重新验证。
func (s *AuthService) VerifyCaptchaForPendingOAuthCreate(ctx context.Context, proof CaptchaProof, remoteIP, verifyCode string) error {
	...
	if providerConfig.Tencent.Enabled || providerConfig.Aliyun.Enabled {
		return s.verifyCaptchaWithConfig(ctx, providerConfig, proof, remoteIP)
	}
	if s.IsEmailVerifyEnabled(ctx) && strings.TrimSpace(verifyCode) != "" {
		logger.LegacyPrintf("service.auth", "%s", "[Auth] Pending OAuth email verify flow detected, skip duplicate Turnstile check")
		return nil
	}
	return s.verifyCaptchaWithConfig(ctx, providerConfig, proof, remoteIP)
}
...
func (s *AuthService) verifyCaptchaWithConfig(ctx context.Context, providerConfig CaptchaProviderConfig, proof CaptchaProof, remoteIP string) error {
