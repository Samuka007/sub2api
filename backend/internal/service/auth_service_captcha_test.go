...
func TestVerifyCaptchaForPendingOAuthCreateSkipsDuplicateTurnstileAfterEmailCode(t *testing.T) {
	...
	svc := newAuthServiceForCaptchaTest(settings, true, verifier, nil)

	err := svc.VerifyCaptchaForPendingOAuthCreate(context.Background(), CaptchaProof{}, "203.0.113.10", "123456")

	require.NoError(t, err)
	require.Zero(t, verifier.called)
}

func TestVerifyCaptchaForPendingOAuthCreateStillVerifiesTencentActionProof(t *testing.T) {
	settings := tencentCaptchaSettings()
	settings[SettingKeyEmailVerifyEnabled] = "true"
	verifier := &tencentCaptchaVerifierStub{response: &TencentCaptchaVerifyResponse{CaptchaCode: 1}}
	svc := newAuthServiceForCaptchaTest(settings, true, nil, verifier)
	...
	err := svc.VerifyCaptchaForPendingOAuthCreate(context.Background(), CaptchaProof{
		TencentTicket:  "create-ticket",
		TencentRandstr: "@create-rand",
	}, "203.0.113.10", "123456")

	require.NoError(t, err)
	require.Equal(t, 1, verifier.calls)
	require.Equal(t, TencentCaptchaProof{Ticket: "create-ticket", Randstr: "@create-rand"}, verifier.proof)
}
