...
func TestContentModerationPreBlockFailsClosedWhenProxyUnavailable(t *testing.T) {
	var direct atomic.Int64
	...
	svc := NewContentModerationService(
		...
		nil,
		nil,
		&contentModerationTestProxyRepo{getByIDErr: errors.New("proxy deleted")},
		nil,
		nil,
	)
	t.Cleanup(func() { svc.Close() })
	...
	if decision.StatusCode != http.StatusServiceUnavailable || decision.Action != ContentModerationActionError {
		t.Fatalf("unexpected fail-closed decision: %+v", decision)
	}
	if direct.Load() != 0 {
		t.Fatal("must not fall back to direct connection when proxy is unavailable")
	}
}

// 每次解析都重新核对代理状态，停用或过期必须立即阻止后续审核流量。
func TestContentModerationProxyURLResolutionReflectsRevocation(t *testing.T) {
	proxy := &Proxy{ID: 3, Name: "p", Protocol: "http", Host: "127.0.0.1", Port: 8080, Status: StatusActive}
	proxyRepo := &contentModerationTestProxyRepo{proxies: map[int64]*Proxy{3: proxy}}
	svc := NewContentModerationService(nil, nil, nil, nil, nil, proxyRepo, nil, nil)
	t.Cleanup(func() { svc.Close() })

	if _, err := svc.resolveModerationProxyURL(context.Background(), 3); err != nil {
		t.Fatalf("resolve active proxy: %v", err)
	}
	proxy.Status = StatusDisabled
	if _, err := svc.resolveModerationProxyURL(context.Background(), 3); err == nil || !strings.Contains(err.Error(), "inactive or expired") {
		t.Fatalf("expected revoked proxy to fail closed, got %v", err)
	}
	if got := proxyRepo.getCalls.Load(); got != 2 {
		t.Fatalf("expected each moderation call to revalidate proxy status, got %d lookups", got)
	}
}
