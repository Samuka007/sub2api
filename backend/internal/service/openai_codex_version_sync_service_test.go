...
func (r *codexVersionSyncSettingRepoStub) CompareAndSet(_ context.Context, key, oldValue, newValue string) (bool, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	if r.setErr != nil {
		return false, r.setErr
	}
	if r.advanceBeforeCAS != "" {
		r.values[key] = r.advanceBeforeCAS
		r.advanceBeforeCAS = ""
	}
...
func (r *codexVersionSyncSettingRepoStub) AdvanceOpenAICodexSyncedVersion(_ context.Context, latest string) (string, bool, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	if r.setErr != nil {
		return "", false, r.setErr
	}
	if enabled := r.values[SettingKeyOpenAICodexVersionAutoSyncEnabled]; enabled != "" && enabled != "true" {
		return "", false, nil
	}
	currentRaw := r.values[SettingKeyOpenAICodexClientVersionSynced]
	if r.advanceBeforeCAS != "" {
		currentRaw = r.advanceBeforeCAS
		r.values[SettingKeyOpenAICodexClientVersionSynced] = currentRaw
		r.advanceBeforeCAS = ""
	}
	if current := NormalizeCodexClientVersion(currentRaw); current != "" && CompareVersions(latest, current) <= 0 {
		return currentRaw, false, nil
	}
	r.values[SettingKeyOpenAICodexClientVersionSynced] = latest
	r.writes = append(r.writes, latest)
	return currentRaw, true, nil
}
...
func (r *codexVersionSyncSettingRepoStub) value(key string) string {
	r.mu.Lock()
	defer r.mu.Unlock()
	return r.values[key]
}
...
func (c *codexVersionSyncGitHubStub) FetchLatestRelease(ctx context.Context, _ string) (*GitHubRelease, error) {
	c.latestCalls++
	if c.latestStarted != nil {
		close(c.latestStarted)
		select {
		case <-c.latestRelease:
		case <-ctx.Done():
			return nil, ctx.Err()
		}
	}
...
func TestOpenAICodexVersionSyncDoesNotOverwriteConcurrentNewerVersion(t *testing.T) {
	repo := newCodexVersionSyncSettingRepoStub(map[string]string{
		SettingKeyOpenAICodexClientVersionSynced: "0.200.0",
	})
	repo.advanceBeforeCAS = "0.220.0"
	github := &codexVersionSyncGitHubStub{
		latest: &GitHubRelease{TagName: "rust-v0.210.0"},
	}

	newCodexVersionSyncService(repo, github).runOnce()

	require.Equal(t, "0.220.0", repo.value(SettingKeyOpenAICodexClientVersionSynced))
	require.Empty(t, repo.syncedWrites())
}

func TestOpenAICodexVersionSyncDoesNotWriteAfterAutoSyncDisabled(t *testing.T) {
	repo := newCodexVersionSyncSettingRepoStub(map[string]string{
	...
	<-github.latestStarted
	repo.mu.Lock()
	repo.values[SettingKeyOpenAICodexVersionAutoSyncEnabled] = "false"
	repo.mu.Unlock()
	close(github.latestRelease)
	<-done

	require.Empty(t, repo.syncedWrites())
}

func TestOpenAICodexVersionSyncStopCancelsInFlightFetch(t *testing.T) {
	repo := newCodexVersionSyncSettingRepoStub(nil)
	github := &codexVersionSyncGitHubStub{
		latestStarted: make(chan struct{}),
		latestRelease: make(chan struct{}),
	...
	case <-time.After(time.Second):
		t.Fatal("Stop did not cancel the in-flight release fetch")
	}
}
...
func (r *blockingCodexVersionSettingRepo) GetMultiple(_ context.Context, keys []string) (map[string]string, error) {
	r.mu.Lock()
	r.calls++
	call := r.calls
	value := r.value
	...
	return map[string]string{SettingKeyOpenAICodexClientVersionSynced: value}, nil
}

func TestOpenAICodexClientVersionInvalidationRejectsStalePublication(t *testing.T) {
	...
	go func() {
		firstDone <- svc.GetOpenAICodexClientVersion(context.Background())
	}()
	<-repo.started
	repo.mu.Lock()
	repo.value = "0.147.0"
	...
	close(repo.release)
	require.Equal(t, "0.146.0", <-firstDone)

	require.Equal(t, "0.147.0", svc.GetOpenAICodexClientVersion(context.Background()))
	repo.mu.Lock()
