...
func openAIWSPassthroughTerminalHasResponseID(payload []byte) bool {
		...
		Options: openaiwsv2.RelayOptions{
			...
			OnTurnComplete: func(turn openaiwsv2.RelayTurnResult) {
				...
				if terminalWriteCompletesTurn {
					...
					terminalWriteCompletesTurn = false
					terminalWriteSucceeded = false
					defer turnLifecycle.finishTerminalWrite(writeSucceeded, clientFrameConn.markTurnCompleted)
				}
			...
			BeforeClientWrite: func(msgType coderws.MessageType, payload []byte) {
				if msgType == coderws.MessageText && openAIWSPassthroughIsTerminalOutput(payload) {
					...
					terminalWriteCompletesTurn = openAIWSPassthroughTerminalHasResponseID(payload)
					terminalWriteSucceeded = false
				}
			},
			AfterClientWrite: func(msgType coderws.MessageType, payload []byte, writeErr error) {
				...
				if isTerminal {
					terminalWriteSucceeded = writeErr == nil
					if !terminalWriteCompletesTurn {
						terminalWriteTurn = 0
						terminalWriteSucceeded = false
			...
			BeforeWriteClient: func(msgType coderws.MessageType, payload []byte, wroteDownstream bool) error {
				...
				if transientModel == "" {
					if sessionModel := sessionUpstreamModel.Load(); sessionModel != nil {
						transientModel = strings.TrimSpace(*sessionModel)
					}
				}
				if isOpenAIWSTerminalEvent(eventType) {
					s.handleOpenAIWSTerminalTransientFailure(ctx, account, transientModel, handshakeHeaders, payload)
				}
				if eventType == "error" {
					s.handleOpenAIWSErrorEventTransientFailure(ctx, account, transientModel, handshakeHeaders, payload)
				}
