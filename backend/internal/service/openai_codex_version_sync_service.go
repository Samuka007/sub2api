...
type codexVersionSyncAdvancer interface {
	AdvanceOpenAICodexSyncedVersion(ctx context.Context, latest string) (previous string, updated bool, err error)
}
...
func (s *OpenAICodexVersionSyncService) Start() {
	...
	go func() {
		...
		s.runInitialContext(s.lifecycleCtx)
		for {
			select {
			case <-ticker.C:
				s.runOnceContext(s.lifecycleCtx)
			case <-s.stopCh:
				return
			}
		}
...
func (s *OpenAICodexVersionSyncService) runInitial() {
	s.runInitialContext(context.Background())
}

func (s *OpenAICodexVersionSyncService) runInitialContext(ctx context.Context) {
	if s.syncedWithinIntervalContext(ctx) {
		return
	}
	s.runOnceContext(ctx)
}

// syncedWithinIntervalContext 判断已同步值是否仍在一个同步周期内。
// 借设置行自身的 UpdatedAt 判断，无需额外记录时间戳的设置项。
// 读取失败或尚无有效同步值时返回 false，让启动同步照常执行。
func (s *OpenAICodexVersionSyncService) syncedWithinIntervalContext(parent context.Context) bool {
	if s.interval <= 0 {
		return false
	}
	ctx, cancel := context.WithTimeout(parent, openAICodexVersionSyncTimeout)
	defer cancel()
...
func (s *OpenAICodexVersionSyncService) runOnce() {
	s.runOnceContext(context.Background())
}

func (s *OpenAICodexVersionSyncService) runOnceContext(parent context.Context) {
	ctx, cancel := context.WithTimeout(parent, openAICodexVersionSyncTimeout)
	defer cancel()
	...
	advancer, ok := s.settingRepo.(codexVersionSyncAdvancer)
	if !ok {
		slog.Error("openai_codex_version_sync_atomic_repository_unavailable")
		return
	}
	previous, updated, err := advancer.AdvanceOpenAICodexSyncedVersion(ctx, latest)
	if err != nil {
	...
	slog.Info("openai_codex_version_synced", "previous", NormalizeCodexClientVersion(previous), "version", latest)
}
