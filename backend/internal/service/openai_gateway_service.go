...
type OpenAIGatewayService struct {
	...
	openaiAccountRuntimeBlockMetadata   sync.Map // key: int64(accountID), value: openAIAccountRuntimeBlockMetadata
...
func NewOpenAIGatewayService(
	...
	// 身份收口函数由 HTTP / 透传 / WS / 探针等出站路径共用，无法在热路径注入配置，
	// 因此在此发布进程级快照。两个配置均取反义，零值即启用保护。
	if cfg != nil {
