...
func TestVerifyCaptchaForPendingOAuthCreateStillVerifiesAliyunActionProof(t *testing.T) {
	settings := aliyunEnabledSettings()
	settings[SettingKeyEmailVerifyEnabled] = "true"
	spy := &aliyunVerifierSpy{}
	svc := newAliyunAuthServiceForTest(&config.Config{
		Server: config.ServerConfig{Mode: "release"},
	}, settings, spy)

	err := svc.VerifyCaptchaForPendingOAuthCreate(
		context.Background(),
		CaptchaProof{TurnstileToken: "aliyun-create-param"},
		"203.0.113.10",
		"123456",
	)
	...
	require.NoError(t, err)
	require.Equal(t, 1, spy.called)
	require.Equal(t, "aliyun-create-param", spy.lastParam)
}
