...
func initializeApplication(buildInfo handler.BuildInfo, modelTrace *modeltrace.Manager) (*Application, error) {
	...
	modelIQService := service.ProvideModelIQService(configConfig)
	modelIQHandler := handler.NewModelIQHandler(modelIQService)
	...
	accountUsageService := service.ProvideAccountUsageService(accountRepository, usageLogRepository, claudeUsageFetcher, geminiQuotaService, antigravityQuotaFetcher, grokQuotaFetcher, grokQuotaService, openAIQuotaService, usageCache, identityCache, geminiTokenCache, tlsFingerprintProfileService, openAIGatewayService)
	...
	quotaRecoveryChecker := service.ProvideQuotaRecoveryChecker(openAIQuotaService, accountUsageService)
	quotaRecoveryService, err := service.ProvideQuotaRecoveryService(accountRepository, quotaRecoveryChecker, openAIGatewayService, configConfig, db)
	if err != nil {
		return nil, err
	}
	opsHandler := admin.ProvideOpsHandler(opsService, quotaRecoveryService)
	...
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, modelIQHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, idempotencyCoordinator, idempotencyCleanupService)
