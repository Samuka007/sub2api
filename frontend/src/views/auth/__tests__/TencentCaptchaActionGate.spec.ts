...
describe('Aliyun captcha action gate', () => {
  beforeEach(() => {
    loginMock.mockReset()
    loginWithPasskeyMock.mockReset()
    getPublicSettingsMock.mockReset()
    ...
    loginWithPasskeyMock.mockResolvedValue({})
    startOAuthLoginMock.mockResolvedValue({ authorize_url: 'https://github.example/authorize' })
    verifyActionMock.mockResolvedValue({ token: 'aliyun-proof' })
    Object.defineProperty(window, 'PublicKeyCredential', {
      configurable: true,
      value: class PublicKeyCredential {}
    })
  ...
  it('maps password proof to turnstile_token', async () => {
    const wrapper = mountLogin()
    await flushPromises()
    await wrapper.get('#email').setValue('user@example.com')
    await wrapper.get('#password').setValue('secret-123')
    ...
    expect(loginMock).toHaveBeenCalledWith(expect.objectContaining({
      turnstile_token: 'aliyun-proof',
      tencent_captcha_ticket: undefined,
      tencent_captcha_randstr: undefined
    }))
  })

  it('maps OAuth and Passkey proofs to turnstile_token', async () => {
    const wrapper = mountLogin()
    await flushPromises()
    await wrapper.get('[data-testid="oauth-start"]').trigger('click')
    await flushPromises()
    ...
    await wrapper.get('button.btn-secondary.w-full').trigger('click')
    await flushPromises()
    expect(loginWithPasskeyMock).toHaveBeenCalledWith({ turnstile_token: 'aliyun-proof' })
  })

  it('does not start password, OAuth, or Passkey actions when proof is cancelled', async () => {
    verifyActionMock.mockResolvedValue(null)
    const wrapper = mountLogin()
    await flushPromises()
    await wrapper.get('#email').setValue('user@example.com')
