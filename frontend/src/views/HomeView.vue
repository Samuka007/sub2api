  ...
  >
    <header class="border-b border-gray-200 px-4 py-4 sm:px-6 dark:border-dark-800">
      <nav class="mx-auto flex max-w-5xl flex-wrap items-center justify-between gap-3 sm:gap-4">
        <div class="flex min-w-0 flex-1 items-center gap-3">
          ...
          />
          <span class="min-w-0 truncate text-base font-semibold">{{ compactSiteName }}</span>
        </div>
    ...
    <main class="flex min-w-0 flex-1 items-center justify-center px-4 py-16 sm:px-6">
      <div class="min-w-0 max-w-2xl text-center">
        ...
        />
        <h1 class="[overflow-wrap:anywhere] text-3xl font-bold md:text-4xl">{{ compactSiteName }}</h1>
    ...
    <footer class="min-w-0 border-t border-gray-200 px-4 py-5 text-center text-sm text-gray-500 [overflow-wrap:anywhere] sm:px-6 dark:border-dark-800 dark:text-dark-400">
      &copy; {{ currentYear }} {{ compactSiteName }}
    </footer>
  </div>

  <div v-else class="robot-home relative h-[100dvh] min-h-[540px] overflow-hidden bg-[#090b12] text-white">
    <img
...
const compactSiteName = computed(
  () => appStore.cachedPublicSettings?.site_name || appStore.siteName || 'SCIbuddy',
)
