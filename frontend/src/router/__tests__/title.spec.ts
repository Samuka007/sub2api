...
describe('resolveDocumentTitle', () => {
  ...
  it('站点名为空时，回退默认站点名', () => {
    expect(resolveDocumentTitle('Dashboard', '')).toBe('Dashboard - SCIbuddy')
    expect(resolveDocumentTitle(undefined, '   ')).toBe('SCIbuddy')
  })

  it('旧默认站点名显示为 SCIbuddy', () => {
    expect(resolveDocumentTitle('Dashboard', 'Sub2API')).toBe('Dashboard - SCIbuddy')
  })
