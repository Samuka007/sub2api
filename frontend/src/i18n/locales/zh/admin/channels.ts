export default {
    ...
    riskControl: {
      ...
      refresh: '刷新',
      config: '内容审计配置',
      configHint: '调用所选内容审核上游进行请求内容评分，命中阈值后按模式处理。',
      openSettings: '内容审计设置',
      ...
      upstreamProtocol: '审核上游协议',
      upstreamProtocolOpenAI: 'OpenAI Moderations',
      upstreamProtocolAnthropic: 'Anthropic Messages',
      upstreamProtocolOpenAIHint: '直接调用 /v1/moderations，使用 OpenAI 兼容的审核响应。',
      upstreamProtocolAnthropicHint: '直接调用 /v1/messages，并通过强制工具调用返回结构化分类分数。',
      baseUrl: '上游 Base URL',
      model: '模型名',
      apiKey: '上游 API Key',
      apiKeys: '上游 API Keys',
      ...
      testingApiKeys: '测试中',
      apiKeyTestNoInput: '请先输入需要测试的上游 API Key',
      apiKeyTestDone: 'Key 测试完成，共 {count} 个',
      apiKeyTestFailed: '测试上游 API Key 失败',
      ...
      trustedAPIKeys: '可信 API Key',
      trustedAPIKeysHint: '命中的 Key 不受本地关键词、历史哈希、模型阈值和安全会话规则阻断，但请求仍会异步审查并留痕。',
      addTrustedAPIKey: '添加可信 Key',
      removeTrustedAPIKey: '移除可信 Key',
      newTrustedAPIKey: '新可信 API Key',
      trustedObserveOnly: '仅观察，不执行本地阻断',
      trustedAPIKeyID: 'API Key ID',
      trustedAPIKeyIDPlaceholder: '服务端数字 ID',
      trustedAPIKeyExpiresAt: '到期时间',
      trustedAPIKeyReason: '放行原因',
      trustedAPIKeyReasonPlaceholder: '说明该 Key 为什么需要可信维护权限',
      trustedAPIKeyModels: '模型范围',
      trustedAPIKeyModelsPlaceholder: '每行一个模型\ngpt-5.6-sol\ncodex-auto-review',
      trustedAPIKeyModelsHint: '留空表示匹配全部模型。',
      trustedAPIKeyEndpoints: '接口范围',
      trustedAPIKeyEndpointsPlaceholder: '每行一个接口\n/v1/responses',
      trustedAPIKeyEndpointsHint: '留空表示匹配全部接口。',
      trustedAPIKeysEmpty: '尚未配置可信 API Key',
      trustedAPIKeysEmptyHint: '仅添加由管理员控制、确实需要在维护时跳过本地阻断的 Key。',
      trustedAPIKeysSecurityNote: '白名单不会绕过身份认证、额度、分组和模型权限、文件系统权限或上游服务商自身的安全策略。Key 被共享或泄露后应立即撤销。',
      trustedAPIKeyInvalidID: '可信 API Key ID 必须是正整数。',
      trustedAPIKeyDuplicate: '可信 API Key ID {id} 被重复配置。',
      trustedAPIKeyReasonRequired: '请填写可信 API Key ID {id} 的放行原因。',
      trustedAPIKeyInvalidExpiry: '可信 API Key ID {id} 的到期时间无效。',
      trustedAPIKeyExpired: '已过期，不再享受本地审查放行',
      trustedAPIKeyScopeTooMany: '可信 API Key ID {id} 的{scope}最多允许 {max} 项。',
      trustedAPIKeyScopeTooLong: '可信 API Key ID {id} 的{scope}单项最多允许 {max} 个字符。',
      tabs: {
        trustedKeys: '可信 Key',
        basic: '基础',
        scope: '审计范围',
