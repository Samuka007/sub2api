export default {
    ...
    riskControl: {
      ...
      configHint: 'Use the selected moderation upstream to score request content and handle threshold hits by mode.',
      ...
      upstreamProtocol: 'Moderation upstream protocol',
      upstreamProtocolOpenAI: 'OpenAI Moderations',
      upstreamProtocolAnthropic: 'Anthropic Messages',
      upstreamProtocolOpenAIHint: 'Calls /v1/moderations directly and consumes an OpenAI-compatible moderation response.',
      upstreamProtocolAnthropicHint: 'Calls /v1/messages directly and uses a forced tool call for structured category scores.',
      baseUrl: 'Upstream Base URL',
      model: 'Model',
      apiKey: 'Upstream API Key',
      apiKeys: 'Upstream API Keys',
      ...
      apiKeyTestNoInput: 'Enter upstream API Keys to test first',
      apiKeyTestDone: 'Key test completed for {count} keys',
      apiKeyTestFailed: 'Failed to test upstream API Keys',
      ...
      trustedAPIKeys: 'Trusted API keys',
      trustedAPIKeysHint: 'Matching keys are never blocked by local keyword, hash, model-threshold, or cyber-session rules. Requests are still audited asynchronously.',
      addTrustedAPIKey: 'Add trusted key',
      removeTrustedAPIKey: 'Remove trusted key',
      newTrustedAPIKey: 'New trusted API key',
      trustedObserveOnly: 'Observe only; local blocking disabled',
      trustedAPIKeyID: 'API Key ID',
      trustedAPIKeyIDPlaceholder: 'Server-side numeric ID',
      trustedAPIKeyExpiresAt: 'Expires at',
      trustedAPIKeyReason: 'Reason',
      trustedAPIKeyReasonPlaceholder: 'Why this key needs trusted maintenance access',
      trustedAPIKeyModels: 'Model scope',
      trustedAPIKeyModelsPlaceholder: 'One model per line\ngpt-5.6-sol\ncodex-auto-review',
      trustedAPIKeyModelsHint: 'Leave empty to match all models.',
      trustedAPIKeyEndpoints: 'Endpoint scope',
      trustedAPIKeyEndpointsPlaceholder: 'One endpoint per line\n/v1/responses',
      trustedAPIKeyEndpointsHint: 'Leave empty to match all endpoints.',
      trustedAPIKeysEmpty: 'No trusted API keys',
      trustedAPIKeysEmptyHint: 'Add only administrator-controlled keys that need maintenance access without local moderation blocking.',
      trustedAPIKeysSecurityNote: 'This list does not bypass authentication, quotas, group/model permissions, filesystem permissions, or upstream provider safety policies. Revoke a key immediately if it is shared or exposed.',
      trustedAPIKeyInvalidID: 'Trusted API Key ID must be a positive integer.',
      trustedAPIKeyDuplicate: 'Trusted API Key ID {id} is configured more than once.',
      trustedAPIKeyReasonRequired: 'A reason is required for trusted API Key ID {id}.',
      trustedAPIKeyInvalidExpiry: 'The expiry for trusted API Key ID {id} is invalid.',
      trustedAPIKeyExpired: 'Expired; local moderation bypass is no longer active',
      trustedAPIKeyScopeTooMany: 'Trusted API Key ID {id} allows at most {max} {scope} entries.',
      trustedAPIKeyScopeTooLong: 'Each {scope} entry for trusted API Key ID {id} is limited to {max} characters.',
      tabs: {
      ...
      action: {
        trustedObserve: 'Trusted observe',
        block: 'Blocked',
