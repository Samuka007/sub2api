...
describe('PendingOAuthCreateAccountForm', () => {
  ...
  it('maps separate Aliyun action proofs to the shared turnstile fields', async () => {
    getPublicSettings.mockResolvedValue({
      email_verify_enabled: true,
      turnstile_enabled: false,
      turnstile_site_key: '',
    ...
    sendPendingOAuthVerifyCode.mockResolvedValue({ countdown: 0 })
    verifyAction
      .mockResolvedValueOnce({ token: 'aliyun-param-1', randstr: '' })
      .mockResolvedValueOnce({ token: 'aliyun-param-2', randstr: '' })
    const CaptchaChallengeStub = defineComponent({
      setup(_, { expose }) {
        expose({ verifyAction, reset: turnstileReset })
        return () => h('div')
  ...
  it('allows submit with the email code after send-code consumes the turnstile proof', async () => {
    getPublicSettings.mockResolvedValue({
      email_verify_enabled: true,
      turnstile_enabled: true,
      turnstile_site_key: 'site-key'
