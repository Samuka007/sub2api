<template>
  <form class="space-y-3" @submit.prevent="handleSubmit">
    ...
    <button
      ...
      :disabled="isSubmitting || !email.trim() || password.length < 6 || (invitationCodeEnabled && !invitationCode.trim()) || (turnstileProofRequiredForSubmit && !turnstileToken)"
      @click="handleSubmit"
    >
...
const turnstileProofRequiredForSubmit = computed(
  () =>
    turnstileEnabled.value &&
    !(emailVerifyEnabled.value && Boolean(verifyCode.value.trim()))
)
...
async function handleSubmit() {
  ...
  // 发码阶段已消费 Turnstile 票据。邮箱验证码填写后，后端以邮箱验证码完成创建阶段门禁，
  // 不再重复消费一次性 Turnstile token；未走邮箱验证时仍必须持有有效票据。
  if (turnstileProofRequiredForSubmit.value && !turnstileToken.value) {
    sendCodeError.value = t('auth.completeVerification')
    return
  }
  ...
  emit('submit', {
    ...
    ...((turnstileProofRequiredForSubmit.value || aliyunCaptchaEnabled.value) && turnstileToken.value
