...
describe('AliyunCaptchaWidget', () => {
  ...
  it('渲染可见验证按钮并以 popup 模式初始化，全局配置就位', async () => {
	...
	it('loads the SDK after publishing global config', async () => {
		delete window.initAliyunCaptcha
		const wrapper = mountWidget()
		const script = document.querySelector<HTMLScriptElement>(
			'script[src*="aliyunCaptcha/AliyunCaptcha"]'
		)
		expect(script?.src).toBe('https://o.alicdn.com/captcha-frontend/aliyunCaptcha/AliyunCaptcha.js')
		expect(window.AliyunCaptchaConfig).toEqual({ region: 'cn', prefix: 'prefix-1' })
		window.initAliyunCaptcha = vi.fn((options: CapturedInitOptions) => {
			initOptions = options
		}) as unknown as typeof window.initAliyunCaptcha
	...
	it('removes a failed SDK script so verify can retry', async () => {
		delete window.initAliyunCaptcha
		const wrapper = mountWidget()
		...
		const widget = wrapper.vm as unknown as { verify: () => Promise<string | null> }
		const verifyPromise = widget.verify()
		await Promise.resolve()
		...
		await expect(verifyPromise).resolves.toBe('captcha-param-retry')
		wrapper.unmount()
	})
